...
import type { WaitlistRow } from '../api';
import { dateRange } from '../format';

export function Waitlist({ rows }: { rows: WaitlistRow[] }) {
  return (
    <Card className="overflow-hidden p-6">
      ...
      <SectionTitle kicker="Waitlist" count={`${rows.length} waiting`} />
      ...
      {rows.length === 0 ? (
        <p className="py-6 text-center font-body text-sm text-muted">No one waiting right now.</p>
      ) : (
